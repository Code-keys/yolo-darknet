...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    // define each layer.
    auto l0 = convBnSilu(network, weightMap, *data, 24, 3, 2, 1, 0);
    auto l1 = convBnSilu(network, weightMap, *l0->getOutput(0), 48, 3, 1, 1, 1);


    auto l2 = convBnSilu(network, weightMap, *l1->getOutput(0), 96, 3, 2, 1, 2);
    auto l3 = convBnSilu(network, weightMap, *l2->getOutput(0), 48, 1, 1, 0, 3);
    auto l4 = convBnSilu(network, weightMap, *l3->getOutput(0), 96, 3, 1, 1, 4);
    auto ew5 = network->addElementWise(*l4->getOutput(0), *l2->getOutput(0), ElementWiseOperation::kSUM);
    auto l6 = convBnSilu(network, weightMap, *ew5->getOutput(0), 48, 1, 1, 0, 6);
    auto l7 = convBnSilu(network, weightMap, *l6->getOutput(0), 96, 3, 1, 1, 7);
    auto ew8 = network->addElementWise(*l7->getOutput(0), *ew5->getOutput(0), ElementWiseOperation::kSUM);
  

    auto l9 = convBnSilu(network, weightMap, *ew8->getOutput(0), 192, 3, 2, 1, 9); 
    auto l10 = convBnSilu(network, weightMap, *l9->getOutput(0), 96, 1, 1, 0, 10);
    auto l11 = l9; 
    auto l12 = convBnSilu(network, weightMap, *l11->getOutput(0), 96, 1, 1, 0, 12);
    auto l13 = convBnSilu(network, weightMap, *l12->getOutput(0), 96, 1, 1, 0, 13);
    auto l14 = convBnSilu(network, weightMap, *l13->getOutput(0), 96, 3, 1, 1, 14);
    auto ew15 = network->addElementWise(*l14->getOutput(0), *l12->getOutput(0), ElementWiseOperation::kSUM);
    auto l16 = convBnSilu(network, weightMap, *ew15->getOutput(0), 96, 1, 1, 0, 16);
    auto l17 = convBnSilu(network, weightMap, *l16->getOutput(0), 96, 3, 1, 1, 17);
    auto ew18 = network->addElementWise(*l17->getOutput(0), *ew15->getOutput(0), ElementWiseOperation::kSUM);
    auto l19 = convBnSilu(network, weightMap, *ew18->getOutput(0), 96, 1, 1, 0, 19);
    auto l20 = convBnSilu(network, weightMap, *l19->getOutput(0), 96, 3, 1, 1, 20);
    ...
    auto l23 = convBnSilu(network, weightMap, *cat22->getOutput(0), 192, 1, 1, 0, 23);

  
    auto l24 = convBnSilu(network, weightMap, *l23->getOutput(0), 384, 3, 2, 1, 24);
    auto l25 = convBnSilu(network, weightMap, *l24->getOutput(0), 192, 1, 1, 0, 25);
    auto l26 = l24;
    auto l27 = convBnSilu(network, weightMap, *l24->getOutput(0), 192, 1, 1, 0, 27);
    auto l28 = convBnSilu(network, weightMap, *l27->getOutput(0), 192, 1, 1, 0, 28);
    auto l29 = convBnSilu(network, weightMap, *l28->getOutput(0), 192, 3, 1, 1, 29);
    auto ew30 = network->addElementWise(*l29->getOutput(0), *l27->getOutput(0), ElementWiseOperation::kSUM);
    auto l31 = convBnSilu(network, weightMap, *ew30->getOutput(0), 192, 1, 1, 0, 31);
    auto l32 = convBnSilu(network, weightMap, *l31->getOutput(0), 192, 3, 1, 1, 32);
    auto ew33 = network->addElementWise(*l32->getOutput(0), *ew30->getOutput(0), ElementWiseOperation::kSUM);
    auto l34 = convBnSilu(network, weightMap, *ew33->getOutput(0), 192, 1, 1, 0, 34);
    auto l35 = convBnSilu(network, weightMap, *l34->getOutput(0), 192, 3, 1, 1, 35);
    ...
    auto l38 = convBnSilu(network, weightMap, *cat37->getOutput(0), 384, 1, 1, 0, 38);


    auto l39 = convBnSilu(network, weightMap, *l38->getOutput(0), 768, 3, 2, 1, 39);

    auto l40 = convBnSilu(network, weightMap, *l39->getOutput(0), 384, 1, 1, 0, 40);
    ...
    auto l47 = convBnSilu(network, weightMap, *cat46->getOutput(0), 768, 1, 1, 0, 47);

    auto l48 = convBnSilu(network, weightMap, *l47->getOutput(0), 384, 1, 1, 0, 48);
    auto l49 = l47;
    auto l50 = convBnSilu(network, weightMap, *l47->getOutput(0), 384, 1, 1, 0, 50);
    auto l51 = convBnSilu(network, weightMap, *l50->getOutput(0), 384, 1, 1, 0, 51);
    auto l52 = convBnSilu(network, weightMap, *l51->getOutput(0), 384, 3, 1, 1, 52);
    ITensor* inputTensorsl53[] = {l52->getOutput(0), l48->getOutput(0)};
    auto cat53 = network->addConcatenation(inputTensorsl53, 2);
    auto l54 = convBnSilu(network, weightMap, *cat53->getOutput(0), 768, 1, 1, 0, 54);


    auto l55 = convBnSilu(network, weightMap, *l54->getOutput(0), 384, 1, 1, 0, 55);

    float *deval = reinterpret_cast<float*>(malloc(sizeof(float) * 384 * 2 * 2));
    for (int i = 0; i < 384 * 2 * 2; i++) {   deval[i] = 1.0;  }
    Weights deconvwts56{DataType::kFLOAT, deval, 384 * 2 * 2};
    IDeconvolutionLayer* deconv56 = network->addDeconvolutionNd(*l55->getOutput(0), 384, DimsHW{2, 2}, deconvwts56, emptywts);
    assert(deconv56);
    deconv56->setStrideNd(DimsHW{2, 2});
    deconv56->setNbGroups(384);
    ITensor* inputTensors57[] = {l38->getOutput(0), deconv56->getOutput(0)};
    auto cat57 = network->addConcatenation(inputTensors57, 2);
    auto l58 = convBnSilu(network, weightMap, *cat57->getOutput(0), 192, 1, 1, 0, 58);
    auto l59 = cat57;
    auto l60 = convBnSilu(network, weightMap, *l59->getOutput(0), 192, 1, 1, 0, 60);
    auto l61 = convBnSilu(network, weightMap, *l60->getOutput(0), 192, 1, 1, 0, 61);
    auto l62 = convBnSilu(network, weightMap, *l61->getOutput(0), 192, 3, 1, 1, 62);
    ITensor* inputTensors63[] = {l62->getOutput(0), l58->getOutput(0)};
    auto cat63 = network->addConcatenation(inputTensors63, 2);
    auto l64 = convBnSilu(network, weightMap, *cat63->getOutput(0), 384, 1, 1, 0, 64);


    auto l65 = convBnSilu(network, weightMap, *l64->getOutput(0), 192, 1, 1, 0, 65);
    
    Weights deconvwts66{DataType::kFLOAT, deval, 192 * 2 * 2};
    IDeconvolutionLayer* deconv66 = network->addDeconvolutionNd(*l65->getOutput(0), 192, DimsHW{2, 2}, deconvwts66, emptywts);
    assert(deconv66);
    deconv66->setStrideNd(DimsHW{2, 2});
    deconv66->setNbGroups(192);
    ITensor* inputTensors66[] = {l23->getOutput(0), deconv66->getOutput(0)};
    auto cat67 = network->addConcatenation(inputTensors66, 2);
    auto l68 = convBnSilu(network, weightMap, *cat67->getOutput(0), 96, 1, 1, 0, 68);
    auto l69 = cat67;
    auto l70 = convBnSilu(network, weightMap, *l69->getOutput(0), 96, 1, 1, 0, 70);
    auto l71 = convBnSilu(network, weightMap, *l70->getOutput(0), 96, 1, 1, 0, 71);
    auto l72 = convBnSilu(network, weightMap, *l71->getOutput(0), 96, 3, 1, 1, 72);
    ITensor* inputTensors73[] = {l72->getOutput(0), l68->getOutput(0)};
    auto cat73 = network->addConcatenation(inputTensors73, 2);
    auto l74 = convBnSilu(network, weightMap, *cat73->getOutput(0), 192, 1, 1, 0, 74);
    ...
    auto l77 = l74;
    auto l78 = convBnSilu(network, weightMap, *l77->getOutput(0), 192, 3, 2, 1, 78);

    ITensor* inputTensors179[] = {l78->getOutput(0), l65->getOutput(0)};
    auto cat79 = network->addConcatenation(inputTensors179, 2);
    auto l80 = convBnSilu(network, weightMap, *cat79->getOutput(0), 192, 1, 1, 0, 80);
    auto l81 = cat79;
    auto l82 = convBnSilu(network, weightMap, *l81->getOutput(0), 192, 1, 1, 0, 82);
    auto l83 = convBnSilu(network, weightMap, *l82->getOutput(0), 192, 1, 1, 0, 83);
    auto l84 = convBnSilu(network, weightMap, *l83->getOutput(0), 192, 3, 1, 1, 84);
    ITensor* inputTensors85[] = {l84->getOutput(0), l80->getOutput(0)};
    auto cat85 = network->addConcatenation(inputTensors85, 2);
    auto l86 = convBnSilu(network, weightMap, *cat85->getOutput(0), 384, 1, 1, 0, 86);  
    ...
    auto l89 = l86;
    auto l90 = convBnSilu(network, weightMap, *l89->getOutput(0), 384, 3, 2, 1, 90);

    ITensor* inputTensors91[] = {l90->getOutput(0), l55->getOutput(0)};
    auto cat91 = network->addConcatenation(inputTensors91, 2);
    auto l92 = convBnSilu(network, weightMap, *cat91->getOutput(0), 384, 1, 1, 0, 92);
    auto l93 = cat91;
    auto l94 = convBnSilu(network, weightMap, *l93->getOutput(0), 384, 1, 1, 0, 94);
    auto l95 = convBnSilu(network, weightMap, *l94->getOutput(0), 384, 1, 1, 0, 95);
    auto l96 = convBnSilu(network, weightMap, *l95->getOutput(0), 384, 3, 1, 1, 96);
    ITensor* inputTensors97[] = {l96->getOutput(0), l92->getOutput(0)};
    auto cat97 = network->addConcatenation(inputTensors97, 2);
    auto l98 = convBnSilu(network, weightMap, *cat97->getOutput(0), 768, 1, 1, 0, 98);  
    ...
    Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, PATH_CALIBRATOR_IMGS , "./int8calib_s.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
    ...
    std::cout  << "Average D2D time: " << (tt / ii) << "ms !"  << std::endl;
