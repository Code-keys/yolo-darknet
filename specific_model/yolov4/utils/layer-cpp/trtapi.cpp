#include "TrtLayerFactory.h"
#include <assert.h>
#include <fstream>


TrtLayerFactory::TrtLayerFactory()
{
}
...
*/
void TrtLayerFactory::print_Weights(Weights wgt, int wgt_size)
{
    for (int i = 0;i < wgt_size; i++) {
        cout<<*((float*)wgt.values + i)<<endl;
    }
}
//e.g! auto input = Build_input(m_network,"data",3,512,512);
ITensor* TrtLayerFactory::Build_input(INetworkDefinition *network, const char *inputName, int C, int H, int W)
{
 ...
 * e.g!
 * auto convlayer = Build_conv(m_networ, *input, wgt, bias, 64, 3, 1, 1);
*/
ITensor* TrtLayerFactory::Build_conv(INetworkDefinition *network, ITensor* input, vector<float> wgt,vector<float> bias, int outputC,
...
/*e.g!
 * auto poollayer = Build_pool(m_network, *input, PoolingType::kMAX, 2, 2, 0);
*/
ITensor* TrtLayerFactory::Build_pool(INetworkDefinition *network, ITensor *input, PoolingType pType, int kernel, int stride, int padding)
{
...
/*e.g!
 * auto batchNorm = Build_batchNorm(network,input,weights,bias,mean,var)
*/
ITensor* TrtLayerFactory::Build_batchNorm(INetworkDefinition *network, ITensor *input, vector<float> weights, vector<float> bias,vector<float> mean,vector<float> var,float eps)
{
...
/*e.g!
 * auto activ = Build_activ(network, input, ActivationType::kRELU);
*/
ITensor* TrtLayerFactory::Build_activ(INetworkDefinition *network, ITensor *input, ActivationType Type, float alpha, float beta)
{
...
ITensor* TrtLayerFactory::Build_FC(INetworkDefinition *network, ITensor *input, int outputC, vector<float> wgt, vector<float> bias)
{
    int size = wgt.size();
 ...
 * auto out = Build_elt(network,input1,input2,ElementWiseOperation::kSUM);
*/
ITensor* TrtLayerFactory::Build_elt(INetworkDefinition *network, ITensor* input1, ITensor* input2, ElementWiseOperation Type)
{
    IElementWiseLayer *elt = network->addElementWise(*input1,*input2,Type);
    return elt->getOutput(0);
}
/**/
ITensor* TrtLayerFactory::Build_concat(INetworkDefinition *network, vector<ITensor *> inputs, int axis)
{
...
ITensor* TrtLayerFactory::Build_slice(INetworkDefinition *network, ITensor *input, Dims start, Dims size, Dims step)
{
    ISliceLayer *slice = network->addSlice(*input,start,size,step);
    return slice->getOutput(0);
}
ITensor* TrtLayerFactory::Build_softmax(INetworkDefinition *network, ITensor *input)
{
    ISoftMaxLayer *softmax = network->addSoftMax(*input);
    return softmax->getOutput(0);
}
ITensor* TrtLayerFactory::Build_MatMul(INetworkDefinition *network, ITensor *input1, bool transpose1, ITensor *input2, bool transpose2)
{
    IMatrixMultiplyLayer *mm = network->addMatrixMultiply(*input1,transpose1,*input2,transpose2);
    return mm->getOutput(0);
}
ITensor* TrtLayerFactory::Build_shuffle_reshape(INetworkDefinition *network, ITensor *input, Dims dims)
{
    IShuffleLayer *reshape = network->addShuffle(*input);
    reshape->setReshapeDimensions(dims);
    return reshape->getOutput(0);
}
ITensor* TrtLayerFactory::Build_shuffle_premute(INetworkDefinition *network, ITensor *input, Permutation pmt)
{
    IShuffleLayer *premute = network->addShuffle(*input);
    premute->setFirstTranspose(pmt);
    return premute->getOutput(0);
}
ITensor* TrtLayerFactory::Build_shuffle(INetworkDefinition *network, ITensor *input, Dims dims, Permutation pmt, bool Reshape_first)
{
...
ITensor* TrtLayerFactory::Build_reduce(INetworkDefinition *network, ITensor *input, uint32_t reduceAxes, ReduceOperation op,bool keepDims)
{
    /*
     *reduceAxes W    H    C
     * 0X01      0    0    1
...
ITensor* TrtLayerFactory::Build_topK(INetworkDefinition *network, ITensor *input, int k, TopKOperation op, uint32_t reduceAxes, int outputAxes)
{
...
ITensor* TrtLayerFactory::Build_upsample(INetworkDefinition *network, ITensor *input, ResizeMode mode, Dims outdims)
{
...
ITensor* TrtLayerFactory::Build_constant(INetworkDefinition *network, Dims dims, vector<float> wgt)
{
    int len = wgt.size();
...
ITensor* TrtLayerFactory::Build_PReLU(INetworkDefinition *network, ITensor* input,vector<float> wgt)
{
    int dimC = input->getDimensions().d[0];
    auto slope = Build_constant(network,Dims3{dimC,1,1},wgt);
    IParametricReLULayer *pReLU = network->addParametricReLU(*input,*slope);
    return pReLU->getOutput(0);
}
ITensor* TrtLayerFactory::Build_groupNorm(INetworkDefinition *network, ITensor *input, int groups, vector<float> wgt, vector<float> bias, float eps)
{
    int dimC = input->getDimensions().d[0];
    if(dimC % groups != 0)
    {
