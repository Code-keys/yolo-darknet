...
# PLUGIN_LIBRARY = os.path.dirname(__file__) + "/lib/unix/libmyplugins.so"
...
class YoLoTRT(object):
    ...
    def __init__(self, engine_file_path,namefile,INPUT_W=640,INPUT_H=640,CONF_THRESH = 0.1,IOU_THRESHOLD = 0.4):
        ...
        self.categories =  self.load_class(namefile)
        self.load_sodll()
    ...
    def draw_detect_results(self, input_image_path):
        # threading.Thread.__init__(self)
    ...
    def load_class(self,namefile):
        with open(namefile,"r") as file:
            clas = file.readlines()
        if clas[-1].split("=").__len__() > 1:
            pass
        else:
            return clas
    def load_sodll(self,):
        if os.name == "nt":
            sys.path.append(os.path.dirname(__file__) + "/model/darknet/yolo.dll")
            sys.path.append(os.path.dirname(__file__) + "/lib/win32/yolo.lib")
        else:
...
if __name__ == "__main__":
    ...
    # a  YoLoTRT instance
    yolov5_wrapper = YoLoTRT(engine_file_path)
    ...
    # for input_image_path in os.listdir("./bin"):

    #     # if not hasattr(image,"shape"):
    #     image = cv2.imread("./bin/"+input_image_path)
    #     # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    #     # create a new thread to do inference
    #     img = yolov5_wrapper.draw_detect_results(image)
    #     cv2.imwrite("./bin/"+input_image_path,img)
