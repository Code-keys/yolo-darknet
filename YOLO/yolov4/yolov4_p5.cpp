...
ILayer* shortCut(INetworkDefinition *network, ITensor& inputL,ITensor& inputR) {
    return network->addElementWise(inputL, inputR, ElementWiseOperation::kSUM);
}
ILayer* sam(INetworkDefinition *network, ITensor& inputL, ITensor& inputR) {
    return network->addElementWise(inputL, inputR, ElementWiseOperation::kSUM);
}
ILayer* splitRoute(INetworkDefinition *network, ITensor* inputTensors[],int num_inputs,int groups, int group_id) {
    ITensor* outputTensors[num_inputs];
    for( int i = 0; ++i; i < num_inputs){
        Dims mDims = Dims3{3,6,6};
        std::cout << mDims.nbDims << mDims.d[0] << mDims.d[1] << mDims.d[2]<< std::endl;
        ITensor* a[1];
        int b;
        int c;
        std::cout << inputTensors[i] ;
        outputTensors[i] = *inputTensors  ;
    };
    return network->addConcatenation(outputTensors, num_inputs);
}
...
void APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream) {
    ...
    ICudaEngine* engine = createEngine_v5s(maxBatchSize, builder, config, DataType::kFLOAT);
