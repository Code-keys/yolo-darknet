...
def obtain_avg_forward_time(input, model, repeat=100):
    model.eval()
    start = time.time()
    with torch.no_grad():
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5m/yolov5m.cfg', help='cfg file path')
    parser.add_argument('--data', type=str, default='data/VOC_Infrared/Ultralytics/infrared.data', help='*.data file path')
    parser.add_argument('--weights', type=str, default='yolov5m/weights/best.pt', help='sparse model weights')
    ...
    # 测试运行速度
    random_input = torch.rand((2, 3, 608, 608)).to(device)
    ...
    # 生成剪枝后的cfg文件并保存模型
    pruned_cfg_name = opt.cfg.replace('.cfg', f'_normalprune_{percent}.cfg')
    ...
    compact_model_name = opt.cfg.replace('.cfg', f'_normalprune_{percent}.weights')
