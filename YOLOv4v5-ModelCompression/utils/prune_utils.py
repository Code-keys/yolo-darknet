...
def parse_module_defs2(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        if module_def['type'] == 'convolutional':
            if module_def['batch_normalize'] and "groups" not in module_def:   # add fix
                CBL_idx.append(i)
            else:
                Other_idx.append(i)
...
def write_cfg(cfg_file, module_defs):
    with open(cfg_file, 'w') as f:
        for indx, module_def in enumerate(module_defs):
            f.write(f"# layer {indx}")
            f.write("\n")
        ...
        if len(route_in_idxs) == 1:
            ...
            if module_defs[route_in_idxs[1] - 1]['type'] == 'maxpool':
                ...
                if module_defs[route_in_idxs[0]]['type'] == 'upsample':
                    ...
                    print(module_defs[route_in_idxs[0]]['type'])
                    print(idx)
